...
const RunModal: React.FC<RunModalProps> = ({ evalItem, models, runModels, handleModelSelection }) => {
  ...
  const handleRunEval = async () => {
    ...
    try {
      const newEval = await addNewEvalRuns(accessToken, evalItem.id, modelSystems);
      ...
      toast({
        ...
        description: "An error occurred while trying to run the eval. Please try again.",
        status: "error",
        isClosable: true,
        duration: 9000,
      });
        ...
        <ModalContent>
          ...
          <ModalBody>
            <VStack spacing={4} align="stretch">
              <Text>Check the accuracy of a model for yourself, while sharing the run results with others!</Text>
              <FormControl>
                <FormLabel>
