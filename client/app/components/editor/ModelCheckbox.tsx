...
export default function ModelCheckbox({
  ...
  const [provider, setProvider] = useState<AIProvider>(
    model.modelDeveloper.toLowerCase() as AIProvider,
  );
  const { openAIKey, anthropicKey, geminiKey, setSettingsVisible } =
    useModelStorageContext();
        ...
        <Popover isLazy isOpen={isOpen} onClose={onClose} onOpen={onOpen}>
          ...
          <PopoverContent>
            <PopoverHeader fontWeight="semibold">
              {AI_PROVIDER_NAME[provider]} key is not set
            </PopoverHeader>
            <PopoverArrow />
            <PopoverCloseButton />
            <PopoverBody>
              <p>
                To run <strong>{model.modelName}</strong>, please set{" "}
                {AI_PROVIDER_A_AN[provider]} {AI_PROVIDER_NAME[provider]} API
                key.
              </p>
              <Button mt={2} onClick={showSettings}>
                Set {AI_PROVIDER_NAME[provider]} key
              </Button>
            </PopoverBody>
          </PopoverContent>
        </Popover>
