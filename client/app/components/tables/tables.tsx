      ...
      <BasicTable
        data={uniqueModels.map((run: IEvalRunResponse) => ({
          ...
          accuracy: (
            ...
            evalRuns.filter((r) => r.model.id === run.model.id).length
          ).toFixed(2),
...
export const ByModel = ({
  ...
  <Box width="100%">
    ...
    <Center mb={8}>
      ...
      >
        {evalRuns
          .filter((run) => !!run.model.modelName.trim())
          .map((run) => (
...
export const ByTaskInstance = ({
  ...
  const selectedTask = taskInstances.find(
    (task) => task.id === selectedTaskInstance,
  );
    ...
    <Box width="100%">
      ...
      <Center mb={8}>
        ...
        <Select
          ...
          fontWeight="bold"
        >
          {Array.from(new Set(taskInstances.map((task) => task.input))).map(
            (input) => {
      ...
      {selectedTaskInstance !== null && (
        <BasicTable
          data={[
            {
              model: "Ideal response",
              response: selectedTask?.ideal || "N/A",
              isCorrect: "N/A",
            },
            ...evalRuns.map((run) => {
              ...
              return {
                ...
                isCorrect: output
                  ? output.output === selectedTask?.ideal
                    ? "Correct"
                    : "Incorrect"
                  : "N/A",
              };
